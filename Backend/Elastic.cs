...
namespace ElasticSearchNamespace
{
    public class ElasticIndex
    {
        ...
        {
            string json = File.ReadAllText(Path.Combine(base_path, "books2.json"));
        ...
        {
            string json = File.ReadAllText(Path.Combine(base_path, "users.json"));
            List<User> users = JsonConvert.DeserializeObject<List<User>>(json);
            return users.ToDictionary(a => a.id);

        }

        public Dictionary<string, Dictionary<string, double>> GetAllUserVectors()
        {
            string json = File.ReadAllText(Path.Combine(base_path, "user_vectors.json"));
            using (var stringReader = new StringReader(json))
            using (var jsonReader = new JsonTextReader(stringReader))
            {
                var serializer = new Newtonsoft.Json.JsonSerializer();
                var userVectors = serializer.Deserialize<Dictionary<string, Dictionary<string, double>>>(jsonReader);
                return userVectors;
            }
        }
